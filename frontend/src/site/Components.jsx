...
import { assets, futureProducts, productFamilies } from './data';
...
export function SolutionCard({ solution }) {
  return (
    <Link className="solution-preview-card" to={`/solutions#${solution.slug}`}>
...
function PhysicalAIStage({ stage }) {
  return (
        ...
        {groups.map((group) => {
          ...
          return (
            ...
            >
              ...
              >
                {group.stages.map((stage) => <PhysicalAIStage stage={stage} key={stage.number || stage.name} />)}
              </ol>
            </GroupElement>
          );
        })}
